		...
		<Headless.Listbox {...props} multiple={false}>
			<Headless.ListboxButton
				...
				className={clsx([
					...
					// Disabled state
					'data-disabled:opacity-50 data-disabled:before:bg-black/5 data-disabled:before:shadow-none',
				])}
			>
				<Headless.ListboxSelectedOption
					...
					className={clsx([
						...
						'min-h-11 sm:min-h-9',
						// Horizontal padding
						'pr-[calc(calc(var(--spacing)*7)-1px)] pl-[calc(calc(var(--spacing)*3.5)-1px)] sm:pl-[calc(calc(var(--spacing)*3)-1px)]',
						// Typography
						'text-left text-base/6 text-black placeholder:text-zinc-500 sm:text-sm/6 forced-colors:text-[CanvasText]',
						// Border
						'border border-black/10 group-data-active:border-black/20 group-data-hover:border-black/20',
						// Background color
						'bg-transparent',
						// Invalid state
			...
			{({ selectedOption }) => {
				...
				return (
					<div
						className={clsx(
							// Basic layout
							'group/option grid cursor-default grid-cols-[calc(var(--spacing)*5)_1fr] items-baseline gap-x-2 rounded-lg py-2.5 pr-3.5 pl-2 sm:grid-cols-[calc(var(--spacing)*4)_1fr] sm:py-1.5 sm:pr-3 sm:pl-1.5',
							// Typography
							'text-base/6 text-black sm:text-sm/6 forced-colors:text-[CanvasText]',
							// Focus
